...
// =============================================================================
void run_benchmark_server(sh_connection_type mode, int use_zero_copy,
                          const char *port_name, benchmark_results_t *results) {
  ...
  shared_host_connection *connection =
  ...
  for (int i = 0; i < LATENCY_SAMPLES; i++) {
    ...
    while (read_from_shared_host_connection(connection, &buffer,
                                            &buffer_size) != SH_OK) {
      _mm_pause();
    }
  ...
  printf("[PHASE 2] Executing Payload Size Sweep (%d ops per size)...\n",
         SWEEP_ITERATIONS);
  printf(" Payload |    Throughput |   Payload BW |      Wire BW |   Avg "
         "Latency\n");
  printf("---------+---------------+--------------+--------------+-------------"
         "-\n");
  ...
  for (size_t s = 0; s < NUM_SWEEP_SIZES; s++) {
    ...
    double elapsed_sec =
        (double)(t_end.QuadPart - t_start.QuadPart) / (double)freq.QuadPart;
    results->ops_per_sec[s] = SWEEP_ITERATIONS / elapsed_sec;
    results->payload_bw_mbps[s] = ((double)SWEEP_ITERATIONS * expected_size) /
                                  (1024.0 * 1024.0 * elapsed_sec);
    results->wire_bw_mbps[s] =
        ((double)SWEEP_ITERATIONS * (expected_size + HEADER_SIZE)) /
        (1024.0 * 1024.0 * elapsed_sec);
  ...
  // =========================================================================
  printf("[PHASE 3] Running Variable-Size Integrity & Boundary Wrap Test (%d "
         "ops)...\n",
         STRESS_ITERATIONS);
  ...
  results->integrity_passed =
      (results->seq_corruptions == 0 && results->data_corruptions == 0);
...
void run_zc_unit_tests(void) {
  ...
  if (zc_write_to_shared_host_connection(NULL, &buf, 64) ==
      SH_ERR_INVALID_PARAMETER) {
  ...
  // Test 3: End-to-end message exchange using zc_write and zc_send (Client to
  // Server)
  tests_total++;
  ...
  shared_host_connection *client_conn =
      (shared_host_connection *)malloc(sizeof(shared_host_connection));
  int err1 = create_shared_host_connection(
      "zc_test_port", (char)SH_FAST_CONNECTION, server_conn);
  size_t size = 0;
  int err2 =
      connect_to_shared_host_connection("zc_test_port", &size, client_conn);

  if (err1 == SH_OK && err2 == SH_OK) {
    // --- Client -> Server Zero-Copy ---
    void *tx_buf = NULL;
    sh_result_t write_res =
        zc_write_to_shared_host_connection(client_conn, &tx_buf, 128);
    if (write_res == SH_OK && tx_buf != NULL) {
      ...
      if (send_res == SH_OK) {
        ...
        sh_result_t read_res =
            read_from_shared_host_connection(server_conn, &rx_buf, &rx_size);
        if (read_res == SH_OK && rx_size == 128 &&
            ((char *)rx_buf)[0] == 0x42 && ((char *)rx_buf)[127] == 0x42) {
          printf(" [PASS] Test 3: zc_write/zc_send end-to-end data exchange\n");
          tests_passed++;
        } else {
          printf(" [FAIL] Test 3: Read back data mismatch or read error "
                 "(res=%d, size=%zu)\n",
                 read_res, rx_size);
        }
      } else {
...
// =============================================================================
void run_benchmark_client(sh_connection_type mode, int use_zero_copy,
  ...
  int retries = 0;
  while (connect_to_shared_host_connection(port_name, &size, connection) !=
         SH_OK) {
    Sleep(1);
    if (++retries > 5000) {
      fprintf(stderr,
              "[CLIENT] Failed to connect to server on port '%s' after 5 "
              "seconds!\n",
              port_name);
      free(connection);
      return;
  ...
  for (int i = 0; i < LATENCY_SAMPLES; i++) {
    if (use_zero_copy) {
      void *buf = NULL;
      while (zc_write_to_shared_host_connection(connection, &buf, 64) !=
             SH_OK) {
        _mm_pause();
      }
      memset(buf, 0, 64);
      ...
      while (write_to_shared_host_connection(connection, payload_64b, 64) !=
             SH_OK) {
        _mm_pause();
      }
    }
  ...
  for (size_t s = 0; s < NUM_SWEEP_SIZES; s++) {
    ...
    if (use_zero_copy) {
      for (int i = 0; i < SWEEP_ITERATIONS; i++) {
        void *buf = NULL;
        while (zc_write_to_shared_host_connection(connection, &buf,
                                                  current_size) != SH_OK) {
          _mm_pause();
        }
      ...
      for (int i = 0; i < SWEEP_ITERATIONS; i++) {
        while (write_to_shared_host_connection(connection, sweep_buf,
                                               current_size) != SH_OK) {
          _mm_pause();
        }
      }
  ...
  for (int i = 0; i < STRESS_ITERATIONS; i++) {
    ...
    if (use_zero_copy) {
      ...
      *(uint64_t *)dynamic_payload = (uint64_t)i;
      ...
      while (write_to_shared_host_connection(connection, dynamic_payload,
                                             packet_size) != SH_OK) {
        _mm_pause();
      }
    }
...
// =============================================================================
// ICEORYX2-STYLE "BULLSHIT TEST": ZERO-COPY WITHOUT MEMORY ACCESS
// =============================================================================

void run_bullshit_test(void) {
  char port_name[32];
  snprintf(port_name, sizeof(port_name), "t_bs_%lu",
           (unsigned long)GetCurrentProcessId());

  shared_host_connection *server_conn =
      (shared_host_connection *)malloc(sizeof(shared_host_connection));
  if (create_shared_host_connection(port_name, SH_FAST_CONNECTION,
                                    server_conn) != SH_OK) {
    free(server_conn);
    return;
  }
  ...
  shared_host_connection *client_conn =
      (shared_host_connection *)malloc(sizeof(shared_host_connection));
  size_t size = 0;
  if (connect_to_shared_host_connection(port_name, &size, client_conn) !=
      SH_OK) {
    close_shared_host_connection(server_conn);
    free(client_conn);
    return;
  ...
  // Drain initial connection handshake "hello" message
  void *buffer = NULL;
  size_t buffer_size = 0;
  server_conn->own_shared_connection_header->last_item_offset = (size_t)-1;
  read_from_shared_host_connection(server_conn, &buffer, &buffer_size);
  client_conn->opp_shared_connection_header->last_item_offset = 0;

  static const size_t BS_SIZES[] = {64, 4096, 65536, 1048576, 1048576 * 64};
  static const int BS_ITERS[] = {100000, 100000, 50000, 500, 100};
  int bs_count = sizeof(BS_SIZES) / sizeof(BS_SIZES[0]);

  printf("=================================================================\n");
  printf(
      "     ICEORYX2-STYLE \"BULLSHIT\" ZERO-COPY BENCHMARK               \n");
  printf("     (Zero-Copy Pointer Passing WITHOUT Memory I/O)              \n");
  printf("=================================================================\n");
  printf(
      " [NOTE] Zero-copy IPC passes 64-bit pointers across shared memory.\n");
  printf("        If payload memory is NEVER written or read (no "
         "memcpy/memset),\n");
  printf("        \"transfer speed\" scales infinitely with payload size!\n\n");
  printf(" Payload Size |    Throughput |  Calculated \"Bandwidth\" | Avg "
         "Latency\n");
  printf("--------------+---------------+-------------------------+------------"
         "--\n");
  fflush(stdout);
  ...
  QueryPerformanceFrequency(&freq);
  double peak_tbps = 0.0;

  for (int s = 0; s < bs_count; s++) {
    size_t payload_sz = BS_SIZES[s];
    int iterations = BS_ITERS[s];

    QueryPerformanceCounter(&t_start);
    for (int i = 0; i < iterations; i++) {
      void *buf = NULL;
      while (zc_write_to_shared_host_connection(client_conn, &buf,
                                                payload_sz) != SH_OK) {
        _mm_pause();
      }
      zc_send_to_shared_host_connection(client_conn);

      size_t open_off = client_conn->open_offset;
      server_conn->own_shared_connection_header->last_item_offset = (size_t)-1;

      while (read_from_shared_host_connection(server_conn, &buffer,
                                              &buffer_size) != SH_OK) {
        _mm_pause();
      }
      client_conn->opp_shared_connection_header->last_item_offset = open_off;
      // CRITICAL: DO NOT TOUCH OR COPY PAYLOAD MEMORY! (The Iceoryx2 special)
    }
    QueryPerformanceCounter(&t_end);

    double elapsed_sec =
        (double)(t_end.QuadPart - t_start.QuadPart) / (double)freq.QuadPart;
    double ops_sec = iterations / elapsed_sec;
    double mb_sec =
        ((double)iterations * payload_sz) / (1024.0 * 1024.0 * elapsed_sec);
    double avg_lat_ns = (elapsed_sec * 1e9) / iterations;

    char bw_str[64];
    if (mb_sec >= 1024.0 * 1024.0 * 1024.0) {
      snprintf(bw_str, sizeof(bw_str), "%.2f PB/s",
               mb_sec / (1024.0 * 1024.0 * 1024.0));
    } else if (mb_sec >= 1024.0 * 1024.0) {
      snprintf(bw_str, sizeof(bw_str), "%.2f TB/s", mb_sec / (1024.0 * 1024.0));
      if (mb_sec / (1024.0 * 1024.0) > peak_tbps)
        peak_tbps = mb_sec / (1024.0 * 1024.0);
    } else if (mb_sec >= 1024.0) {
      snprintf(bw_str, sizeof(bw_str), "%.2f GB/s", mb_sec / 1024.0);
    } else {
      snprintf(bw_str, sizeof(bw_str), "%.1f MB/s", mb_sec);
    }

    char size_str[32];
    if (payload_sz >= 1073741824)
      snprintf(size_str, sizeof(size_str), "%zu GB", payload_sz / 1073741824);
    else if (payload_sz >= 1048576)
      snprintf(size_str, sizeof(size_str), "%zu MB", payload_sz / 1048576);
    else if (payload_sz >= 1024)
      snprintf(size_str, sizeof(size_str), "%zu KB", payload_sz / 1024);
    else
      snprintf(size_str, sizeof(size_str), "%zu B", payload_sz);

    printf(" %12s | %11.0f/s | %14.1f MB/s (%s) | %8.1f ns\n", size_str,
           ops_sec, mb_sec, bw_str, avg_lat_ns);
    fflush(stdout);
  }

  double safe_verdict_tbps = 1.92;
  if (peak_tbps > 0.0 && peak_tbps < 100000.0) {
    safe_verdict_tbps = peak_tbps;
  }

  printf("\n [VERDICT] %.2f TERABYTES/sec bandwidth achieved!\n",
         safe_verdict_tbps);
  printf(" (Because zero bytes of data were actually written to or read from "
         "RAM!)\n");
  printf(
      "=================================================================\n\n");
  fflush(stdout);
