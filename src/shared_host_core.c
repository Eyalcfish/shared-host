...
sh_result_t write_to_shared_host_connection(shared_host_connection* connection, void* buffer, size_t buffer_size) {
    ...
    size_t current_item_offset = *(size_t*)(last_item_address); // the address of the last item's next item offset, aka our current item offset's pointer

    if (current_item_offset + buffer_size + 2*sizeof(size_t) >= ring_buffer_size) { // check if the offset of the the current item + buffer_size + 2*sizeof(size_t) is bigger than the ring buffer size
        //check if we can write from page_start
        if (connection->opp_shared_connection_header->current_item_offset >= buffer_size + 2*sizeof(size_t)) { // we can write from the start, aka check if we can write the message inbetween 0 and the reader's current read item
    ...
    *(size_t*)current_item_address = current_item_offset + buffer_size + 2*sizeof(size_t); // this sets the next item offset to the current item offset + buffer_size + 2*sizeof(size_t)
    *(size_t*)((char*)current_item_address + sizeof(size_t)) = buffer_size; // this sets the size of the current item

    memcpy((void*)((char*)current_item_address + 2*sizeof(size_t)), buffer, buffer_size); // this copies the buffer to the current item address + 2*sizeof(size_t) for headers
